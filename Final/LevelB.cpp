...
// ------ CAMERA ADJUST ---------//
float leftb_bound_offset = -(LEVEL_WIDTH * 0.5) / 2.0f;
float topb_bound_offset = (LEVEL_HEIGHT * 0.5) / 2.0f;
glm::vec3 viewb_offset(leftb_bound_offset, topb_bound_offset, 0.0f);
...
void LevelB::initialise()
{
    ...
    m_game_state.map = new Map(LEVEL_WIDTH, LEVEL_HEIGHT, LEVELB_DATA, map_texture_id, 0.5f, 7, 6);
    m_game_state.towers = new Map(LEVEL_WIDTH, LEVEL_HEIGHT, TOWERB_DATA, tower_texture_id, 0.5f, 12, 11);
    ...
    m_game_state.player->set_position(-viewb_offset + (glm::vec3(-7.3f, 0.0f, 0.0f)));
    ...
    m_game_state.P1cursor->set_position(-viewb_offset + (glm::vec3(-6.0f, -1.0f, 0.0f)));
    m_game_state.P2cursor->set_position(-viewb_offset + (glm::vec3(6.0f, -1.0f, 0.0f)));
    ...
    m_game_state.fireball1->set_position(-viewb_offset + (glm::vec3(-6.5f, -0.5f, 0.0f)));
    ...
    m_game_state.fireball2->set_position(-viewb_offset + (glm::vec3(6.1f, -0.5f, 0.0f)));
    ...
    m_game_state.fireball2->set_acceleration(glm::vec3(0.0f, -4.9f, 0.0f));
    ...
    //m_game_state.blocker2 = new Entity(Utility::load_texture("assets/Paddle.png"), 0.5f, 0.35f, 0.5f, BLOCKER);

    m_game_state.blocker1->set_position(-viewb_offset + (glm::vec3(0.0f, 0.0f, 0.0f)));
    //m_game_state.blocker2->set_position(-viewb_offset + (glm::vec3( 6.65f, -0.05f, 0.0f)));

    m_game_state.blocker1->set_starting_pos(-viewb_offset + (glm::vec3(-6.5f, -0.05f, 0.0f)));
    //m_game_state.blocker2->set_starting_pos(-viewb_offset + (glm::vec3(6.65f, -0.05f, 0.0f)));
    ...
    m_game_state.blocker1->set_jumping_power(10.0f);
...
void LevelB::update(float delta_time)
{
   ...
   /* Entity** collidables = new Entity * [2];
    ...
    //m_game_state.blocker2->update(delta_time, m_game_state.player, m_game_state.fireball1, ENEMY_COUNT, m_game_state.map, m_game_state.towers);
}
